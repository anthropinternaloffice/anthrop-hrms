...
| | In the backup? |
|---|---|
| Every table: people, employments, departments, job titles, attendance, audit log | **Yes** |
| Roles and grants (`00-roles.sql`) | **Yes** — without them, policies restore but nobody can read anything |
| Row-level security policies, triggers, functions | **Yes**, part of the schema dump |
| **Employee documents — the actual files** | **No** |
| **Login accounts in `auth.users`** | **No** |

The last two matter enormously and are the reason to read this in advance.

**Documents.** The files live in the `employee-documents` Storage bucket, which is not part
of a database dump. A restore gives you every *record* of a document — who uploaded what, and
when — and none of the files. If Anthrop needs those recoverable too, that is a separate job
and it has not been built. Say so now rather than discovering it during an incident.

**Logins.** `auth` is managed by Supabase and is not included. After a restore, `profiles`
rows point at auth user ids that do not exist in the new project. The data is all there; the
ability to sign in is not. Rebuilding access means recreating the users and repointing
`profiles.id`, which is the fiddliest part of a real recovery.
...
2. **Recreate the logins.** `auth.users` is not in the backup. Create each person in
   Authentication → Users, then update `public.profiles.id` to the new auth user ids. Start
   with the Owner or nobody can administer anything.
3. **The documents are not coming back from this.** Say so early to whoever is waiting.
4. **Repoint the application**: `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` in
   ...
   changes nothing. See `docs/deployment.md`.
